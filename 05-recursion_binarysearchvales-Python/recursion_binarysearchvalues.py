...
def binarySearchValues(x,y,v,L):
	if(x<=y):
		mid = (x+y)//2	
		if(L[mid]>v):
			return [(mid,L[mid])]+binarySearchValues(x,mid-1,v,L)
		if(L[mid]<v):
			return [(mid,L[mid])]+binarySearchValues(mid+1,y,v,L)
		if(L[mid]==v):
			return [(mid,L[mid])]
	else:
		return []				
...
def recursion_binarysearchvalues(L, v):
	...
	return binarySearchValues(0,len(L)-1,v,L)
